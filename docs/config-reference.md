...
variables are reserved when DCGM power telemetry is enabled.

Custom benchmarks can use the shared DCGM power collector by setting `telemetry.enabled: true`,
`benchmark.client_placement: head`, and a nonempty list of unique
...
Custom power does not support dedicated frontend or benchmark-client nodes.
