...
class Nodes:
    ...
    def from_slurm(
        ...
        Args:
            ...
            batch_host_as_head: Keep head/client on the collector batch host.
            batch_host: Authoritative SLURMD_NODENAME for custom power.
        """
        dedicated_roles = [
            role
            for role, wanted in (
        ...
        if het_lists is not None:
            ...
            return cls._from_het_slurm(het_lists, etcd_nats_dedicated_node, batch_host_as_head, batch_host)
        ...
        if batch_host_as_head:
            if frontend_dedicated_node or client_dedicated_node:
                raise ValueError("custom power requires frontend and benchmark client on the batch host")
            head = cls._resolve_batch_host(batch_host, nodelist)
            if etcd_nats_dedicated_node:
                ...
                infra, worker = head, tuple(nodelist)
            return cls(head=head, bench=head, infra=infra, worker=worker)
...
class RuntimeContext:
    ...
    def from_config(
        ...
        batch_host_as_head = config.telemetry.enabled and config.benchmark.type == "custom"
