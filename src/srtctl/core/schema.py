...
class SrtConfig:
    ...
    def _validate_dcgm_power(self):
        ...
        if self.benchmark.type != _BENCHMARK_TYPE_SA_BENCH and not custom_agentx:
            raise ValidationError(
                f"telemetry requires benchmark.type: {_BENCHMARK_TYPE_SA_BENCH} or {_BENCHMARK_TYPE_CUSTOM}"
            )
        if custom_agentx and (self.frontend.dedicated_node or self.benchmark.client_dedicated_node):
            raise ValidationError("custom power requires frontend and benchmark client on the batch host")
        ...
        if custom_agentx and placement_options:
            raise ValidationError(
                "telemetry with benchmark.type: custom does not allow "
        ...
        if custom_agentx and reserved_slurm_env:
            raise ValidationError(
                "telemetry with benchmark.type: custom reserves environment keys "
        ...
        if custom_agentx and reserved_env:
            raise ValidationError("telemetry reserves benchmark.env keys: " + ", ".join(reserved_env))
